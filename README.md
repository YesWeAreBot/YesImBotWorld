...
- **实时调用**：总览同时显示 Bot / World 的流式输出，独立调用页按发起时间排列请求。通过稳定调用 ID 查看实际发送的完整 JSON 请求体、原始 JSON / SSE 返回和 HTTP 错误；生成中的返回持续追加，暂停跟随时保持选中调用。调试摘要不再充当完整原文。
- **运行洞察 / 模型用量**：按来源、级别、模型和时间筛选真实记录，点击图表查看对应样本，检查事件因果与缓存统计。
