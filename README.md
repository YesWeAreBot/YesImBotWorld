...
- **总览操作**：管理员可在总览页直接使用本插件的 `world.*` 操作按钮，按需展开参数表单和执行记录，与 Koishi 共用执行逻辑，不发送指令消息。刷新及切页保留表单草稿和执行状态；旧 `#commands` 地址兼容转到总览。
...
世界属性、事件、观测和工具回执默认展开为可读字段与列表，保留原始字段名和精确的字符串值。嵌套数据可以展开，长列表和长文本按需加载更多；原始 JSON 仍可展开和复制。
