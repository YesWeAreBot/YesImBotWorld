...
操作设备前，管理员先点击「接管」，工作室会暂停 Bot 自主操作并等待已有调用结束。之后的网页操作使用同一套设备工具和真实回执；完成后交还控制。聊天发送需在页面明确提交。远程桌面需要已有 VNC 配置；虚拟世界里的电脑会明确显示为模拟模式。接口与模式说明见 [设备工作台文档](docs/webui-devices.md)。
