...
启用 `webui.enabled` 后，通过 `world.webui` 查看地址。工作室支持桌面、平板和手机，提供深浅主题、快捷导航及 `Ctrl/⌘ K` 搜索。图标、世界插画、设备图形和图表均由项目内的 SVG 绘制，不依赖外部美术资源。

- **世界关系图**：按地点、角色、物件筛选，拖动和缩放空间图；选择节点查看位置、所有权、属性可见性、关联行动与事务因果。
- **角色与成长**：沿着支持、反证和修订查看认识的形成过程，点击证据查看角色实际收到的观测。
- **设备工作台**：共享 Bot 当前的手机与电脑。聊天、天气、浏览器、记事本、新闻和 MCP 各有独立界面；电脑按实际模式提供终端、文件工具或远程桌面。
- **走进世界**：填写角色卡作为独立角色入场，或选择「仅操纵身体 / 完全入替 Bot」。常驻角色驾驶舱按实际开放能力生成表单，共用 Bot 的工具执行路径，展示观测、执行中调用、取消和图文回执；设备自动继承角色模式。
- **世界指令**：管理员可直接执行本插件的 `world.*` 指令。结构化参数、执行状态与历史回执直接显示在网页，与 Koishi 共用执行逻辑，不发送指令消息。
- **运行洞察**：合并实时调用和调试分析。顶部持续显示 Bot / World 流式输出，下方切换「调用原文」与「事件与图表」。通过稳定调用 ID 查看实际 JSON 请求体、原始 JSON / SSE 返回和 HTTP 错误；生成中的返回持续追加。旧 `#debug` 地址兼容转到 `#live`。
...
总览向访问者问候，常驻 Bot 卡片自动读取 Koishi 平台头像，优先匹配当前聊天使用的账号。实时调用原文只保存在内存中，最多 200 次调用、合计 32 MB、单次 8 MB；超限会明确标记不可用。原文沿用 `debug` 查看权限，不包含 Authorization 请求头。
...
- 驾驶舱使用后端当前开放的工具 Schema，应用/频道切换后同步能力目录。等待和休息按参数中的 TU 计时；其他工具的预计世界秒会换算为 TU。可用高级 JSON 提交复杂参数，发送工具需明确确认。
...
- 开启：通过 function calling 声明当前可用工具，每次请求恰好一个调用。应用原始 JSON Schema 保留嵌套结构、枚举与必填字段；关闭应用后工具失效。
...
WebUI 只展示运行时实际使用的模板。结构化 World 裁定使用 `adjudicationSystem`，只读应用呈现使用 `presentationSystem`；创世辅助判定与记忆压缩模板保持独立。`observe`、到达/离开等确定性操作没有额外的旧版叙事模板。

旧 `system`、`send_event` / Markdown 更新及访客叙事模板已停用，不会迁入新的裁定提示。读取配置不改写原文件；首次保存时，含停用模板的原始覆盖备份到 `webui/prompts.legacy.json`。有效的用户覆盖保留，需在编辑器中「恢复默认」才采用更新后的内置文本。保存成功后用于后续生成，正在运行的请求不改写。
...
以事件展开用法，并动态更新允许列表及原生工具声明（关闭/离开后失效）：
