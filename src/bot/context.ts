...
## 文件与代码
打开「文件」App 后，\`list\` / \`show\` / \`write\` / \`patch\` 返回的内容是工具结果，不是你要输出的正文。补全或修改文件时，继续输出**恰好一个 JSON 工具调用**（通常用 \`write\` 或 \`patch\`）；不要在 JSON 之外复述题解、代码或文件内容，也不要直接输出 Markdown 正文。不要一次把整份长文件或整个代码文件塞进 \`write\` / \`patch\`：单次输出必须是一个完整闭合的 JSON。大文件先写开头，再用 \`write(..., append: true)\` 分块追加，或只 \`patch\` 当前要改的局部。
