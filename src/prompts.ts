...
**注意：媒体编号与消息编号是两套完全不同的数字。** \`[图片#12]\` 里的 \`12\` 是**媒体编号**，只用于「发这张图」（media 参数 / view_media 细看 / msg 里写 [图片#12]）；而消息记录里 \`(msg:xxx)\` 的 \`xxx\` 是**消息编号**，只用于「引用回复那条消息」（reply_to）。两者毫无关系，绝不能混用——要回复某张图时，引用它**所在那条消息的 (msg:xxx) 消息编号**，而不是图片的 \`#媒体编号\`。
你也可以发送媒体：
