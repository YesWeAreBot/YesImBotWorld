...
function renderStateEditor(s){
  ...
  panes.appendChild(jsonlPane('Bot 小事记', 'Bot 的私人小事（facts.jsonl），可固定条目——固定后重置世界/重新创世也会保留', s.facts || [], '/api/state/facts', '新增一条 Bot 小事…', {sortable: true, pinnable: true}));
  ...
  function render(){
    ...
    ordered().forEach(function(e){
      ...
      it.appendChild(el('div', {cls:'toolbar', style:'margin:4px 0 0'}, [
        ...
        opts.pinnable ? el('button', {cls: n.pinned ? 'primary' : '', title: n.pinned ? '取消固定：重置世界/重新创世后这条仍会保留' : '固定这条：重置世界/重新创世也不会删除', text: n.pinned ? '取消固定' : '固定', onclick:function(){
          api('POST', urlBase + '/pin', {index: i, pinned: !n.pinned}).then(function(){ n.pinned = !n.pinned; toast(n.pinned ? '已固定（重置/创世后保留）' : '已取消固定', 'ok'); render(); }).catch(showErr);
        }}) : null
      ]));
      list.appendChild(it);
    });
