...
import type { DraftSegment, MediaRef, MediaType, PickFailure, PickResult, RichText, RichTextPart } from "../types.js";
...
export class KoishiMessenger implements MessengerApi {
  ...
  /**
   * 发送输入框（draft）的 segments：media/文件/语音段已经过 pick_media 解析成具体 MediaRef，
   * 无需再走 INLINE_MEDIA / media 参数解析。text 段仍走富文本解析（@/表情标签）。
   * 复用 send 的冷频道拦截 + 表情包分批 + 留痕逻辑。
   */
  async sendSegments(
    id: string,
    ...
    // 依序展开 segments：text 走富文本，media 走对应元素
    for (const seg of segments) {
      if (seg.kind === "text") {
        await pushText(seg.text);
      } else if (seg.kind === "media") {
        const mediaEl = await this.mediaElement(seg.ref, seg.sticker);
        if (seg.sticker) stickerElements.push(mediaEl);
        else elements.push(mediaEl);
        sentRefs.push(seg.ref);
        if (seg.sticker) stickerStored += mediaPlaceholder(seg.ref.id, seg.ref.type);
        else stored += mediaPlaceholder(seg.ref.id, seg.ref.type);
      } else if (seg.kind === "file") {
        // 文件段：读文件并作为 file 元素发送
        const data = await this.media.readFile(seg.ref);
        elements.push(this.fileElement("file", data, seg.ref.mime, seg.label));
        stored += `[文件 ${seg.label}]`;
      } else {
        // 语音段：TTS 合成后作为 audio 元素发送
        if (!this.tts) {
          problems.push("当前没有可用的语音合成能力，语音段未发出");
          continue;
        }
        try {
          const audio = await this.tts.speech(seg.text);
          elements.push(h("audio", { src: toDataUrl(audio.data, audio.mime) }));
          stored += `[语音] ${seg.text}`;
        } catch (err) {
          problems.push(`语音合成失败：${(err as Error).message ?? err}`);
        }
      }
    }
