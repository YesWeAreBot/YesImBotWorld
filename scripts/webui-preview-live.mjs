...
export function createLiveFixture(publish) {
  ...
  function step(body) {
    ...
    else if(body.action==='evict'){buffers.delete(id);call.rawAvailable=false;call.rawUnavailableReason='开发样本：原始记录已按容量淘汰';}
  ...
  return {
    list(){return{calls:structuredClone([...calls.values()]),retention:{maxCalls:200,maxBytes:33554432,maxCallBytes:8388608,persistent:false}};},
